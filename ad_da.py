...
Edward Springer, 2019-03-18; DAC changes
"""
import time
import struct
...
class AD_DA(object):
    ...
    def ads_v_ref(self):
        ...
        """
        return self._ads_v_ref
    @ads_v_ref.setter
    def ads_v_ref(self, value):
        self._ads_v_ref = value

    # DAC8532 Voltage Reference
    @property
    def dac_v_ref(self):
        return self._dac_v_ref
    @dac_v_ref.setter
    def dac_v_ref(self, value):
        self._dac_v_ref = value

    # Pi specific voltage magnitude
    @property
    def v_mag(self):
        return self._v_mag
    @v_mag.setter
    ...
    def v_per_digit(self):
        ...
        """
        return self.ads_v_ref * 2.0/(self.pga_gain * (2**23 - 1))
    @v_per_digit.setter
    ...
    def voltage_convert(self, voltage):
        return np.ushort(65536 * voltage / self.dac_v_ref)
    ...
    def __init__(self, conf=default_config):
        ...
        self.ADS_CS_PIN   = conf.ADS_CS_PIN
        self.DRDY_TIMEOUT = conf.DRDY_TIMEOUT
        self.DRDY_DELAY   = conf.DRDY_DELAY

        # DAC configuration
        self.DAC_CS_PIN     = conf.DAC_CS_PIN
        self.DAC_CHANNEL_A  = conf.DAC_CHANNEL_A
        self.DAC_CHANNEL_B  = conf.DAC_CHANNEL_B
        ...
        self.ads_v_ref  = conf.ads_v_ref
        self.dac_v_ref  = conf.dac_v_ref
        self.v_mag      = conf.v_mag
    ...
    def _chip_select(self, cs):
        ...
        if cs is not None:
            wp.digitalWrite(cs, wp.LOW)

    # Release chip select and implement t_11 timeout
    def _chip_release(self, cs):
        if cs is not None:
    ...
    def _send_int24(self, data):
        # Python3 only:
        wp.wiringPiSPIDataRW(self.SPI_CHANNEL, int.to_bytes(data, 3, "big"))
    ...
    def write_dac(self, channel, data):
        self._chip_select(self.DAC_CS_PIN)
        self._send_uint8(channel)
        self._send_uint8(data >> 8)
        self._send_uint8(data & 0xFF)
        self._chip_release(self.DAC_CS_PIN)
